...
var Servidor="./objeto/objAutocompletarNombre2.php"; // Determina la pagina donde buscar
...
function Contenido(idContenido2)
{
	...
	if(Conexion.status==200) // Si conexion HTTP es buena !!!
	{
		...
		if(Conexion.responseText)
		{
			...
			*/
			document.getElementById(idContenido2).style.display="block";
			document.getElementById(idContenido2).innerHTML=Conexion.responseText;
		}else
			document.getElementById(idContenido2).style.display="none";
	}else{
		document.getElementById(idContenido2).innerHTML=Conexion.status+"-"+Conexion.statusText;
	}
...
function Solicitud(idContenido2,Cadena)
{
	...
	if(Cadena && Cadena!=Palabra)
	{
		...
		if(Conexion)
		{
			...
			*	false - Sincronizado */
			...
			Conexion.send("idContenido2="+idContenido2+"&word2="+Cadena+"&"+date.getTime());
		}else
			document.getElementById(idContenido2).innerHTML="No disponible";
	}
}

// Funcion que inicia la busqueda.
// Tiene que recibir el identificador donde mostrar el listado, y la cadena a buscar
function autocompletarNombre2(idContenido2,Cadena)
{
	...
	if(Cadena.length>=1)
	{
		...
		Solicitud(idContenido2,Cadena);
	}else
		document.getElementById(idContenido2).style.display="none";
}

// Funcion que se ejecuta cuando seleccionamos un valor del desplegable
function selectItem2(idContenido2,value,cod)
{
	...
	document.getElementById("txtCodNombre").value=cod;
	
	//volvemos a indicar que actualice el listado con el nuevo valor
	autocompletarNombre2(idContenido2,value);
}
