...
var Servidor="./objeto/objAutocompletarProfesion.php"; // Determina la pagina donde buscar
...
function Contenido(idContenido3)
{
	...
	if(Conexion.status==200) // Si conexion HTTP es buena !!!
	{
		...
		if(Conexion.responseText)
		{
			...
			*/
			document.getElementById(idContenido3).style.display="block";
			document.getElementById(idContenido3).innerHTML=Conexion.responseText;
		}else
			document.getElementById(idContenido3).style.display="none";
	}else{
		document.getElementById(idContenido3).innerHTML=Conexion.status+"-"+Conexion.statusText;
	}
...
function Solicitud(idContenido3,Cadena)
{
	...
	if(Cadena && Cadena!=Palabra)
	{
		...
		if(Conexion)
		{
			...
			*	false - Sincronizado */
			...
			Conexion.send("idContenido3="+idContenido3+"&word3="+Cadena+"&"+date.getTime());
		}else
			document.getElementById(idContenido3).innerHTML="No disponible";
	}
}

// Funcion que inicia la busqueda.
// Tiene que recibir el identificador donde mostrar el listado, y la cadena a buscar
function autocompletarProfesion(idContenido3,Cadena)
{
	...
	if(Cadena.length>=1)
	{
		...
		Solicitud(idContenido3,Cadena);
	}else
		document.getElementById(idContenido3).style.display="none";
}

// Funcion que se ejecuta cuando seleccionamos un valor del desplegable
function selectItem(idContenido3,value,cod)
{
	// Cuando pulsamos sobre el desplegable, colocamos el valor en el cuadro de texto
	document.getElementById("txtProfesion").value=value;
	document.getElementById("txtCodProfesion").value=cod;
	
	//volvemos a indicar que actualice el listado con el nuevo valor
	autocompletarProfesion(idContenido3,value);
}
